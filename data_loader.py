...
# 同ChannelAug中的ChannelRandomErasing
...
# 使用预处理好的数据构建SYSU数据集：加载训练图像、标签，并进行必要的数据增强，最后将处理好的图像与标签以合适的形式返回给训练或测试代码
class SYSUData(data.Dataset):
    def __init__(self, transform=None, colorIndex = None, thermalIndex = None, data_dir = None, data_dir1 = None):
        ...
        # 用torchvision.transforms
        # 进行标准化、随机裁剪、水平翻转、以及自定义的通道增强（ChannelAdapGray和ChannelRandomErasing）
        # 使用平均值和标准差对张量图像进行归一化。
        ...
        self.transform_thermal = transforms.Compose( [
            ...
            # 将PIL图像再转换为张量
            # convert a PIL image/ndarray to tensor (H*W*C) in range [0,255] to a torch.
            # Tensor(C*H*W) in the range [0.0,1.0]
            transforms.ToTensor(),
            # 归一化
            normalize,
            # ChannelAug中的像素随机擦除操作
            ChannelRandomErasing(probability = 0.5),
            # 随机选择通道进行复制，还能够将彩色图像转换为灰度图像
    ...
    def __getitem__(self, index):
        # 提取索引index在各个图像集下对应的图像和标签,并用assert检查两种模态下图像的标签是否与形状变换后的标签一致。
        ...
        # img1：对RGB图像应用数据增强。
        # img2：对红外图像应用数据增强。
        # img1_shape：对形状变换后的RGB图像应用简单的数据增强。
        # img2_shape：对形状变换后的红外图像应用简单的数据增强。
        img1 = trans_rgb(img1)
...
# 构建VCM数据集（视频序列）因此比起RegDB和SYSU-MM01需要的处理更加复杂
class VCM(object):
...
class TestData(data.Dataset):
    def __init__(self, test_img_file, test_label, test_cam, transform=None, img_size = (144,288)):
        # 对图像文件进行处理，裁剪为指定大小然后转换为np.array的test_image形式
        test_image = []
